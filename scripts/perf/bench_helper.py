...
def parse_output(filepath):
    with open(filepath, 'r') as f:
        ...
        lines = f.readlines()[-2:] # Grab last two lines of bench output
...
if __name__ == '__main__':
    ...
    parser.add_argument('--log_x',
                        default=False,
                        action="store_true",
                        help="Flag to enable log base 2 scaling on x-axis (if disabled, then linear)")
    ...
    for file in args.bench_logs:
        ...
        results[batch] = parse_output(file)

        if args.text_output:
            print(f'Batch Size = {batch}')
            print(f'Average time = {results[batch][0]}\nAverage gflops = {results[batch][1]}\n')
