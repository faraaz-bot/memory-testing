...
# Recursive Cooley-Tukey, assuming length is a power of 2, DIT
...
def icooley_tukey_pow2(x):
    ...
    print(N)
    # Length log(N) loop:
    for s in range(1, int(np.log(N) / np.log(2)) + 1):
        a = int(pow(2, s))
        b = int(N / a)
        print(a, b)
        # Length-N loop:
        for l in range(b): # how many loops
            halfa = int(a / 2)
            for k in range(halfa): # size of loop
                # 2 elements per loop
                p = l * a + k
                q = l * a + k + int(a / 2)
                print("\t", p, q )
