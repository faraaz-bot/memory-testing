...
typedef void (*TestCall)(int*, bool, int*);
...
void solution_0(int* d_data, bool coop_launch, int* d_counters)
{
    int b_stride = LEN;
    int c_stride = LEN;
...
void solution_1(int* d_data, bool coop_launch, int* d_counters)
{
...
void solution_2(int* d_data, bool coop_launch, int* d_counters)
{
    if(!coop_launch)
    {
...
void solution_3(int* d_data, bool coop_launch, int* d_counters)
{
...
void solution_4(int* d_data, bool coop_launch, int* d_counters)
{
                                                 ...
                                                 int*      counters)
{
    ...
    while(done != 1)
    {
        ...
        {
            task_id   = atomicAdd(&counters[0], 1);
    ...
    if(threadIdx.x == 0)
    {
        while(atomicAdd(&counters[1], 0) < 0)
        {
        }
    }
    __syncthreads();
    ...
    while(done != 1)
    {
        if(threadIdx.x == 0)
        {
            task_id   = atomicAdd(&counters[1], 1);
            l_task_id = task_id;
        }
...
void solution_5(int* d_data, bool coop_launch, int* d_counters)
{
    if(!coop_launch)
    {
        plus_one_twice_sync_tasks_atomic<<<dim3(LEN * LEN), dim3(LEN)>>>(
            d_data, LEN, LEN, d_counters);
    }
    else
    {
        int   b_stride = LEN;
        int   c_stride = LEN;
        void* kernelArgs[]
            = {(void*)&d_data, (void*)&b_stride, (void*)&c_stride, (void*)&d_counters};
    ...
    for(int coop_launch = 0; coop_launch <= 1; coop_launch++)
    {
        ...
        for(auto i = 0; i < SOLUTION_NUM; i++)
        {
            if(i != 4)
            {
                ...
                device_malloc((void**)&d_counters, sizeof(int) * TRIAL_NUM * 2);
                device_memcpy_h2d(d_counters, h_counters, sizeof(int) * TRIAL_NUM * 2);

                device_event_create();

                // warm up once
                solution[i](d_data, coop_launch, d_counters);

                device_event_record_start();

                for(int j = 1; j < TRIAL_NUM - 1; j++)
                {
                    solution[i](d_data, coop_launch, &d_counters[j * 2]);
                }
