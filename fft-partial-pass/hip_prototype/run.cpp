...
static constexpr size_t timing_no_trials = 20;
static constexpr double data_empty_val = -123456789;
...
void ComputeRandomInputBuffer(const size_t N,
                              ...
                              gpubuf_t<T> &input)
{
    std::vector<T> input_host(N * nbatch);
    ...
    auto input_size_bytes = N * nbatch * sizeof(T);
...
template <typename T>
void ComputeInputBuffer(size_t N, size_t nbatch, gpubuf_t<T> &input)
{
    std::vector<T> input_host(N * nbatch);
    ...
    for (auto ibatch = 0; ibatch < nbatch; ++ibatch)
    {
        for (auto i = 0; i < N; ++i)
        {
            input_host[idata].x = i;
            input_host[idata].y = i;
...
template <typename T>
void InitializeOutputBuffer(size_t N, size_t nbatch, gpubuf_t<T> &output)
{
    std::vector<T> output_host(N * nbatch);

    for (auto &elem : output_host)
    {
        elem.x = data_empty_val;
        elem.y = data_empty_val;
    }

    auto output_size_bytes = N * nbatch * sizeof(T);
...
struct CS_3D_RC_64_64_64_Params
{
    CS_3D_RC_64_64_64_Params()
    {
        ...
        Get_SBRR_64_Args_PP<T>(length, inStride_1, outStride_1, iDist, oDist, batch, lds_padding, gridParam1_PP, devKernArg1_PP);
        Get_SBCC_64_Args_PP<T>(length, inStride_3, outStride_3, iDist, oDist, batch, lds_padding, gridParam3_PP, devKernArg3_PP);

        if (ibuf.alloc(N * batch * sizeof(rocfft_complex<T>)) != hipSuccess)
            throw std::runtime_error("hipMalloc failure");
        ComputeRandomInputBuffer(N, length, inStride_1, iDist, batch, ibuf);

        if (tbuf.alloc(N * batch * sizeof(rocfft_complex<T>)) != hipSuccess)
            throw std::runtime_error("hipMalloc failure");

        if (obuf.alloc(N * batch * sizeof(rocfft_complex<T>)) != hipSuccess)
            throw std::runtime_error("hipMalloc failure");
        InitializeOutputBuffer(N, batch, obuf);
    }
    ...
    GridParam gridParam1_PP;
    GridParam gridParam3_PP;

    gpubuf_t<size_t> devKernArg1;
    gpubuf_t<size_t> devKernArg2;
    gpubuf_t<size_t> devKernArg3;

    gpubuf_t<size_t> devKernArg1_PP;
    gpubuf_t<size_t> devKernArg3_PP;

    gpubuf_t<rocfft_complex<T>> ibuf;
    gpubuf_t<rocfft_complex<T>> tbuf;
...
{
    std::vector<T> buffer_host(N * nbatch);

    auto buffer_size_bytes = N * nbatch * sizeof(T);
...
void Run_CS_3D_RC_64_64_64_PP(bool measure_exec, bool print_input)
{
    ...
    for (unsigned int itrial = 0; itrial < gpu_time.size(); ++itrial)
    {
        ...
        Launch_3D_FFT_PP(csParams.length,
                         DFT_matrix_large,
                         csParams.twiddles1_RR,
                         csParams.twiddles3_CC,
                         csParams.lds_padding,
                         csParams.gridParam1_PP,
                         csParams.gridParam3_PP,
                         csParams.devKernArg1_PP,
                         csParams.devKernArg3_PP,
                         csParams.batch,
                         csParams.callbacks,
                         csParams.ibuf,
                         csParams.tbuf,
