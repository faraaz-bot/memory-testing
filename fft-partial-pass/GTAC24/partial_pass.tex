...
Fast Fourier Transforms (FFTs) are memory-bound workloads on modern GPU accelerators. We present a new method to optimally distribute FFT work among compute kernels based on the amount of available shared memory. Rearranging FFT work in this fashion allow us to reduce the total number of passes through global memory, which is frequently the bottleneck in GPU-based FFT algorithms. For rocFFT, the software library for computing FFTs in AMD's ROCm ecosystem, the reduction in global memory transactions directly translates to improved performance for many HPC customers critical workloads.
...
Let $\mathbf{y} = \mathcal{F}\left\{ \mathbf{x} \right\}$ denote the one-dimensional discrete Fourier transform (DFT) of $\mathbf{x}$, which maps an $N$-length input sequence $\mathbf{x} = \begin{bmatrix} x_0 &  \cdots & x_{N-1} \end{bmatrix}$ into an $N$-length output sequence $\mathbf{y} = \begin{bmatrix} y_0 & \cdots & y_{N-1} \end{bmatrix}$ with
\begin{equation}
...
For the multidimensional case, let $d$ denote the number of dimensions with $N=N_1,\cdots,N_d$. If we treat the $N$-length input and output sequences $\mathbf{x}$ and $\mathbf{y}$ as $d$-dimensional arrays
\begin{equation*}
\begin{split}
& \mathbf{x} \coloneq \mathbf{x}(0 \colon N_1-1,\ \cdots,\ 0 \colon N_1-1) \\
& \mathbf{y} \coloneq \mathbf{y}(0\colon N_1-1,\ \cdots,\ 0 \colon N_1-1), 
\end{split}
\end{equation*}
...
In this work, we focus on complex-valued entries for the input and out sequences. The computational complexity of directly evaluating the DFT scales as $O(N^2)$. 
...
Each DFT in a given stage is completely independent from each other, thus the entire FFT work of a stage is parallelizable. This shows that FFTs are clearly embarrassingly parallel workloads. Each stage requires writing intermediate results back to memory. Since global memory bandwidth is significantly lower than the GPU compute units bandwidth, the FFT passes or stages are performed entirely in shared memory. With this approach, only a single trip to global memory is required. The amount of available shared memory is, therefore, a determining factor for the performance of FFTs in GPUs. FFTs are clearly memory bound workloads on these devices, since each DFT in an FFT stage typically has low arithmetic intensity.
...
%TODO: Separability property of the DFT. 3d ffts means 1d fft along each dimension. Define multi-dimensional array and present the dft formula pp. 148 of van loan. Practical implementation involve transposes for column or row oriented FFTs. 3d data as blocked matrices?

\subsection{FFT work decomposition}

Consider the case where an FFT of length $N$ does not fit into shared memory. If this FFT is to be performed entirely in global memory, then $\log(N)$ round-trips to global memory are required if a radix-$2$ implementation is used, or $k$ round-trips with $N=q_1 \times q_2 \times \cdots \times q_k$ if a mixed-radix FFT is carried out. This compute scheme, however, would be generally slow and inefficient on modern GPUs since global memory is significant slower than the compute units. 

We now describe a well-known procedure that allows a length-$N$ FFT to be decomposed into a series of smaller FFTs. Let $\mathbf{F}_N (0 \colon N_1-1, 0 \colon N_2-1)$ with $N = N_1 \times N_2$ denote an $N_1 \times N_2$ submatrix of the DFT matrix $\mathbf{F}_N$ obtained by picking the $0$ to $N_1-1$ rows and the $0$ to $N_2-1$ columns of $\mathbf{F}_N$. Similarly, let $\mathbf{X}_{N_1 \times N_2}$ denote an $N_1 \times N_2$ matrix obtained by a column major-ordering of $\mathbf{x}$, i.e., by picking every consecutive $N_1$ entries of $\mathbf{x}$ and setting them as columns of $\mathbf{X}_{N_1 \times N_2}$. The following 4-step algorithm can be used to perform a large, length-$N$ FFT with $N = N_1 \times N_2$, in terms of two smaller FFTs of length $N_1$ and $N_2$:
\begin{algorithm}
\begin{algorithmic}[1]
\State $\mathbf{X}_{N_1 \times N_2} \gets \mathbf{X}_{N_1 \times N_2} \mathbf{F}_{N_2}$ 
\State $\mathbf{X}_{N_1 \times N_2} \gets \mathbf{F}_n (0 \colon N_1-1, 0 \colon N_2-1) .* \mathbf{X}_{N_1 \times N_2}$ 
\State $\mathbf{X}_{N_2 \times N_1} \gets \mathbf{X}_{N_1 \times N_2}^{T}$
\State $\mathbf{X}_{N_2 \times N_1} \gets \mathbf{X}_{N_2 \times N_1} \mathbf{F}_{N_1}$ 
\end{algorithmic}
\caption{4-Step FFT Algorithm \cite{bailey89}}
\label{alg:4_step_fft}
\end{algorithm}

Steps 1 and 4 perform the smaller FFTs, $N_1$ FFTs of length $N_2$ along the rows of $\mathbf{X}_{N_1 \times N_2}$, and $N_2$ FFTs of length $N_1$ along the rows of $\mathbf{X}_{N_2 \times N_1}$, respectively. Step 2 involves the Hadamard product between the matrices $\mathbf{F}_n (0 \colon N_1-1, 0 \colon N_2-1)$ and $\mathbf{X}_{N_1 \times N_2}$, while step 3 performs a matrix transposition. If any of the smaller FFTs do not fit into shared memory, the decomposition can be applied recursively until the smaller FFTs satisfy the shared memory size constraints.

In terms of the GPU implementation of this decomposition, multiple round-trips to global memory may be required due to the necessity to load and store intermediate results. However, the total number of passes through global memory is significantly reduced compared to performing the entire FFT within global memory. For example, in rocFFT, if both the smaller FFTs of length $N_1$ and $N_2$ satisfy the shared memory size constraints in the first decomposition of $N$, then an additional round-trip to global memory is required. This is assuming the best case scenario where the Hadamard product and the transposition steps are fused into the two compute kernels that perform the smaller FFTs. In other words, the first compute kernel loads the input data from global memory, perform steps 1 and 2 in shared memory, and stores the intermediate results into global memory, while the second compute kernel loads the intermediate results from global memory, perform steps 3 and 4, and stores the final result into global memory. If the length $N_1$ or $N_2$ FFTs do not satisfy the shared memory constraints in the first decomposition of $N$, then the procedure is applied recursively until a smaller enough FFT is reached, thus requiring multiple additional compute kernels and global memory accesses.


\section{Optimal Distribution of FFT Work}

In rocFFT, we are constrained by a shared-memory of size $65,536$ bytes on AMD's MI accelerator series. For single-precision numbers, $8$ bytes are used and since we are dealing with complex-valued arithmetic, we have $16$ bytes per complex-valued element in an FFT. This results in a maximum FFT length of $N_{\max} = 4,096$ for single-precision, one-dimensional FFTs that fit entirely into shared memory. Similarly, for double-precision the maximum length is $N_{\max} = 2,048$. Let $\mathcal{N} = \left\{ 1, \cdots, N_{\max} \right\}$ denote the set of all one-dimensional FFTs in rocFFT that fit entirely into shared memory. These FFT compute kernels can be interpreted as building blocks upon which larger FFTs can be computed. 
...
$$
  T = \frac{(n+k-1)!}{k!(n-1)!},
$$
where $k=3$ and $n=193$. Plugging in n and k gives us a total of $T=1,216,865$ three-dimensional problems to explore.
