...
\begin{figure*}[h]  
  ...
  \caption{Median execution time for single-precision (left) and double-precisoin (right).}
...
\begin{figure*}[h]  
    ...
    \captionsetup[sub]{singlelinecheck=off}
    \subfloat[$\mkern-90mu$]{\label{subfig:single_precision_speedup} \includegraphics[width=0.45\textwidth]{singlespeedup.pdf}}
    \subfloat[$\mkern-90mu$]{\label{subfig:double_precision_speedup} \input{double_precision_speedup.tikz}}
    \caption{Speedup for single-precision (left) and double-precisoin (right).}
    \label{fig:speed_up}
  \end{figure*}
