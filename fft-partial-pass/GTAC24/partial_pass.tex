...
off-dimension in three-dimensional FFTs, the total number of
...
$64$KiB on AMD's instinct accelerator series. Compute kernels in rocFFT use
a little over $32$KiB bytes of shared memory, due to additional
...
maximum FFT length of $N_{\max} = 4,096$ for single-precision, FFTs
that fit entirely into shared memory. Similarly, for double-precision
the maximum length is $N_{\max} = 2,048$. Let $\mathcal{N} = \left\{
1, \cdots, N_{\max} \right\}$ denote the set of all one-dimensional
...
% three-dimensional FFT of length $(64,64,64)$. This workload has
% been previously carried out with three compute kernels, but now with
% the proposed method we are able to use two compute kernels. 

% The performance improvements of the proposed 2 kernel partial-pass algorithm compared to the default 3 kernel algorithm have been evaluated with AMD's Instinct accelerator series (MI100, MI250X, MI300A, and MI300X). The proposed partial-pass algorithm will be publicly available in the upcoming ROCm 6.4 release for the three-dimensional length $(64,64,64)$ FFT. The default algorithm is currently available on ROCm 6.3. Both algorithms have been run $50$ times on each Instinct card, and the execution time in miliseconds was recorded for each run. 

% \Cref{fig:execution_time} compares the median execution time of partial-pass and default algorithms over $50$ runs with the Instinct accelerators. \Cref{subfig:single_precision_time} and \Cref{subfig:double_precision_time} show results single and double-precision FFTs, respectively. In both figures, we vary the batch size from $1$ to $10,000$. This is equivalent to datasets of size $$ %TODO: mention out place and temp buffers in the total memory calculation. Explain batch meaning as well.
...
\begin{figure}[h]  
  ...
  \subfloat[$\mkern-90mu$]{\label{subfig:single_precision_time} \input{single_precision_time.tikz}} \quad \quad
...
\begin{figure}[h]  
    ...
    \captionsetup[sub]{singlelinecheck=off}
    \subfloat[$\mkern-90mu$]{\label{subfig:single_precision_speedup} \input{single_precision_speedup.tikz}} \quad \quad
