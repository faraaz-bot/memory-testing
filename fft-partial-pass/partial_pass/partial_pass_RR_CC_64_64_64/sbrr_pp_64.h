...
__device__ void forward_length64_SBRR_device_(scalar_type *R,
...
extern "C" __global__
    __launch_bounds__(64) void fft_rtc_fwd_len64_factors_8_8_wgs_64_tpt_8_dim3_dp_ip_CI_unitstride_sbrr_pp(
        ...
        scalar_type *__restrict__ buf)
{
    ...
    size_t idx = 0, offset_new = 0, remaining_new = 0;
    ...
    remaining = transform;
    remaining_new = 64 * (transform / 64) + (transform % 64) / 8 + (transform * 8) % 64;
    for (int d = 1; d < dim; ++d)
    {
        ...
        index_along_d = remaining_new % lengths[d];
        remaining_new = remaining_new / lengths[d];
        offset_new = offset_new + index_along_d * stride[d];
    }
    batch = remaining;
    // offset = offset + batch * stride[dim];
    offset_new = offset_new + batch * stride[dim];
    ...
    if (inbound)
    {
        ...
        idx = offset_new + (thread + 0) * stride0;
        lds_complex[offset_lds + (thread + 0)] = load_cb(buf, idx, load_cb_data, nullptr);

        idx = offset_new + (thread + 8) * stride0;
        lds_complex[offset_lds + (thread + 8)] = load_cb(buf, idx, load_cb_data, nullptr);

        idx = offset_new + (thread + 16) * stride0;
        lds_complex[offset_lds + (thread + 16)] = load_cb(buf, idx, load_cb_data, nullptr);

        idx = offset_new + (thread + 24) * stride0;
        lds_complex[offset_lds + (thread + 24)] = load_cb(buf, idx, load_cb_data, nullptr);

        idx = offset_new + (thread + 32) * stride0;
        lds_complex[offset_lds + (thread + 32)] = load_cb(buf, idx, load_cb_data, nullptr);

        idx = offset_new + (thread + 40) * stride0;
        lds_complex[offset_lds + (thread + 40)] = load_cb(buf, idx, load_cb_data, nullptr);

        idx = offset_new + (thread + 48) * stride0;
        lds_complex[offset_lds + (thread + 48)] = load_cb(buf, idx, load_cb_data, nullptr);

        idx = offset_new + (thread + 56) * stride0;
        lds_complex[offset_lds + (thread + 56)] = load_cb(buf, idx, load_cb_data, nullptr);
    }
    ...
    //
    // this dim: length-64
    //   uses x threads per row-transform of length-64
    //   does 8 row-transforms per thread block
    // therefore it uses x active threads
    ...
    lds_to_reg_input_length64_device_<scalar_type, SB_1ST>(
        R, lds_complex, stride_lds, offset_lds, thread_in_device, true);

    // transform
    forward_length64_SBRR_device_<scalar_type,
                                  lds_is_real,
                                  SB_1ST,
                                  lds_linear,
                                  direct_load_to_reg>(
    ...
    lds_from_reg_output_length64_device_<scalar_type, SB_1ST>(
        R, lds_complex, stride_lds, offset_lds, thread_in_device, true);

    // =======================================================================================================
    // Partial Pass (Steps 1 and 2) - length-8 DFT on off-dimension and Hadamard product with twiddle factors
    // =======================================================================================================

    auto stride_lds_pp = 64;
    auto offset_lds_pp = (blockIdx.x * 8 + threadIdx.x) % 64;
    ...
    //twiddle multiply
    twiddle_multiple_PP<scalar_type>(R, blockIdx.x % 8, DFTmatrix_large);
