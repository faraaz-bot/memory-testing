...
__device__ void forward_length64_SBCC_device_(scalar_type *R,
...
extern "C" __global__
    __launch_bounds__(64) void fft_rtc_fwd_len64_factors_8_8_wgs_64_tpt_8_dim3_dp_ip_CI_sbcc_dirReg_intrinsicRead_pp(
        const scalar_type *__restrict__ twiddles,
        const scalar_type *large_twiddles,
        ...
        scalar_type *__restrict__ obuf)
{
    ...
    if (in_bound)
    {
        global_idx = (offset + tid_hor * stride[1]) + (thread + 0) * stride0;
        lds_complex[tid_hor_lds * stride_lds + (thread_lds + 0) * 1] = load_cb(ibuf,
                                                                               global_idx,
                                                                               load_cb_data,
                                                                               nullptr);
        global_idx = (offset + tid_hor * stride[1]) + (thread + 8) * stride0;
        lds_complex[tid_hor_lds * stride_lds + (thread_lds + 8) * 1] = load_cb(ibuf,
                                                                               global_idx,
                                                                               load_cb_data,
                                                                               nullptr);
        global_idx = (offset + tid_hor * stride[1]) + (thread + 16) * stride0;
        lds_complex[tid_hor_lds * stride_lds + (thread_lds + 16) * 1] = load_cb(ibuf,
                                                                                global_idx,
                                                                                load_cb_data,
                                                                                nullptr);
        global_idx = (offset + tid_hor * stride[1]) + (thread + 24) * stride0;
        lds_complex[tid_hor_lds * stride_lds + (thread_lds + 24) * 1] = load_cb(ibuf,
                                                                                global_idx,
                                                                                load_cb_data,
                                                                                nullptr);
        global_idx = (offset + tid_hor * stride[1]) + (thread + 32) * stride0;
        lds_complex[tid_hor_lds * stride_lds + (thread_lds + 32) * 1] = load_cb(ibuf,
                                                                                global_idx,
                                                                                load_cb_data,
                                                                                nullptr);
        global_idx = (offset + tid_hor * stride[1]) + (thread + 40) * stride0;
        lds_complex[tid_hor_lds * stride_lds + (thread_lds + 40) * 1] = load_cb(ibuf,
                                                                                global_idx,
                                                                                load_cb_data,
                                                                                nullptr);
        global_idx = (offset + tid_hor * stride[1]) + (thread + 48) * stride0;
        lds_complex[tid_hor_lds * stride_lds + (thread_lds + 48) * 1] = load_cb(ibuf,
                                                                                global_idx,
                                                                                load_cb_data,
                                                                                nullptr);
        global_idx = (offset + tid_hor * stride[1]) + (thread + 56) * stride0;
        lds_complex[tid_hor_lds * stride_lds + (thread_lds + 56) * 1] = load_cb(ibuf,
                                                                                global_idx,
                                                                                load_cb_data,
                                                                                nullptr);
    }
    ...
    // =======================================================================================================
    // Partial Pass (Steps 3 and 4) - local transposition and length-8 DFT on off-dimension
    // =======================================================================================================

    auto stride_lds_pp = 64;
    auto offset_lds_pp = threadIdx.x;

    // call a pre-load from lds to registers (if necessary)
    lds_to_reg_input_length64_device_PP<scalar_type>(R, lds_complex, stride_lds_pp, offset_lds_pp);

    // Radix-8 pass
    FwdRad8B1(R + 0, R + 1, R + 2, R + 3, R + 4, R + 5, R + 6, R + 7);

    // call a post-store from registers to lds (if necessary)
    lds_from_reg_output_length64_device_PP<scalar_type>(R, lds_complex, stride_lds_pp, offset_lds_pp);

    // Local transposition
    tid_hor *= 8;

    // Recalculate global memory idx
    auto offset_tid_hor = (offset + tid_hor * stride[1]);
    offset_tid_hor = (offset_tid_hor + (offset / 512) * (64 * 64 - 448)) % (64 * 64);

    // =======================================================================================================

    // calc the thread_in_device value once and for all device funcs
    auto thread_in_device = lds_linear ? threadIdx.x % 8 : threadIdx.x / 8;

    // call a pre-load from lds to registers (if necessary)
    lds_to_reg_input_length64_device_<scalar_type, lds_linear ? SB_UNIT : SB_NONUNIT>(
        R, lds_complex, stride_lds, offset_lds, thread_in_device, true);

    // transform
    forward_length64_SBCC_device_<scalar_type,
                                  lds_is_real,
    ...
    // call a post-store from registers to lds (if necessary)
    lds_from_reg_output_length64_device_<scalar_type, lds_linear ? SB_UNIT : SB_NONUNIT>(
        R, lds_complex, stride_lds, offset_lds, thread_in_device, true);
    ...
    if (in_bound)
    {
        global_idx = offset_tid_hor + (thread + 0) * stride0;
        store_cb(obuf,
                 global_idx,
                 lds_complex[tid_hor_lds * stride_lds + (thread_lds + 0) * 1],
                 store_cb_data,
                 nullptr);
        global_idx = offset_tid_hor + (thread + 8) * stride0;
        store_cb(obuf,
                 global_idx,
                 lds_complex[tid_hor_lds * stride_lds + (thread_lds + 8) * 1],
                 store_cb_data,
                 nullptr);
        global_idx = offset_tid_hor + (thread + 16) * stride0;
        store_cb(obuf,
                 global_idx,
                 lds_complex[tid_hor_lds * stride_lds + (thread_lds + 16) * 1],
                 store_cb_data,
                 nullptr);
        global_idx = offset_tid_hor + (thread + 24) * stride0;
        store_cb(obuf,
                 global_idx,
                 lds_complex[tid_hor_lds * stride_lds + (thread_lds + 24) * 1],
                 store_cb_data,
                 nullptr);
        global_idx = offset_tid_hor + (thread + 32) * stride0;
        store_cb(obuf,
                 global_idx,
                 lds_complex[tid_hor_lds * stride_lds + (thread_lds + 32) * 1],
                 store_cb_data,
                 nullptr);
        global_idx = offset_tid_hor + (thread + 40) * stride0;
        store_cb(obuf,
                 global_idx,
                 lds_complex[tid_hor_lds * stride_lds + (thread_lds + 40) * 1],
                 store_cb_data,
                 nullptr);
        global_idx = offset_tid_hor + (thread + 48) * stride0;
        store_cb(obuf,
                 global_idx,
                 lds_complex[tid_hor_lds * stride_lds + (thread_lds + 48) * 1],
                 store_cb_data,
                 nullptr);
        global_idx = offset_tid_hor + (thread + 56) * stride0;
        store_cb(obuf,
                 global_idx,
                 lds_complex[tid_hor_lds * stride_lds + (thread_lds + 56) * 1],
                 store_cb_data,
                 nullptr);
    }
