...
\begin{frame}
  \frametitle{Statistical tests: Lockhart MI250X A1 with CrayOS}

   \begin{center}
     \includegraphics[width=0.9\textwidth]{asy/x1000c0s2b0n0_fpr.pdf}
   \end{center}
...
\begin{frame}
  ...
  \item FPR increases with more samples. \pause
...
\begin{frame}
    ...
    \item Use minimal orthogonal set of tests
      \medskip
    \item Get enough samples ($\geq{}16$? $\geq{}32$?)
      \medskip
...
\begin{frame}
  ...
  \item Using longitudinal data
    \medskip
  \item Testing on other libraries / hardware
    \medskip
...
\begin{frame}
  ...
  The data should look piece-wise constant - discontinuities imply
  performance changes.
...
\begin{frame}
  \frametitle{Other Libraries / Hardware}
...
\begin{frame}
  ...
  \begin{itemize}
  \item This model is known as a multi-armed bandit.
  \item The solution algorithm is called Thompson Sampling.
  \item We can apply Bayesian techniques to reduce test time.
  \end{itemize}
...
\begin{frame}
  ...
  I'd like to thank Kai Samuel Lukowiak and Sam Yeaman for their
  helpful discussions about statistics amd the rocFFT team for their
  help abusing dload!
