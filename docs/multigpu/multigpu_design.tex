...
\end{lstlisting}
for all $d$ dimensions and $P$ devices.  This struct is equivalent to
what is used in FFTW's guru interface, so users may be more familiar
with it (and we can also use this for single-GPU higher-dimension
...
rocfft_status rocfft_plan_description_set_device_layout(
  ...
  size_t rocfft_iodim_size,
  rocfft_iodim* rocfft_iodim_s);
rocfft_status rocfft_plan_description_get_device_layout(
  rocfft_plan_description description,
  size_t device_num,
  size_t dimension,
  rocfft_iodim* rocfft_iodims);
