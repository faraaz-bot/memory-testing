...
In multiple dimensions, say $D$,
$\mathbf{k}=\left(k_1,\dots,k_D\right)$, $\mathbf{n}=\left(n_1,\dots,n_D\right)$, and
$\mathbf{N}=\left(N_1,\dots,N_D\right)$ are vector-valued.  If $x_{\mathbf{n}}$ is real-valued,
then $X_{\mathbf{k}}$ is complex with Hermitian symmetry, in that
\begin{dmath}
...
incorrect results, since the output is symmetrized and half of it is
the complex conjugate of the expected result, albeit in a different
location in memory.
...
Hermitian symmetry is then re-imposed by re-indexing the data and
...
dimensions are roughly equal in magnitude, then the symmetrize-to-even
...
\begin{algorithm}
  \caption{$D$-dimensional real-to-complex FFT using the symmetrize-to-even method}\label{DFFTold}
  \begin{algorithmic}[1]
