...
\begin{frame}
  ...
  specific to certain problem size factorizations.

  \medskip
  
  In this talk, I will present some optimisation plans which would
  greatly benefit from the inclusion of runtime compilation in the
  \texttt{rocFFT} library.

  \bigskip

  These optimsations are as follows:
  
  \bigskip
  \begin{itemize}
  \item Increase Problem Size Coverage for Stockham Kernels
    \medskip
  \item Eliminate Uesless Work for Bluestein Kernels
    \medskip
  \item Fuse Kernels for Multdimensional Transforms
  \end{itemize}
...
\begin{frame}
  \frametitle{Problem Size Coverage}
    ...
    FFTs are computed by dividing a problem of lenght $N$ into smaller
...
\begin{frame}
  ...
  zeros from legnth $N$ to at least length $2N$.
...
\begin{frame}[fragile]
  ...
  Most of these optimsations are largely infeasible, or must be reduced
