...
\begin{frame}
  ...
  \item Eliminate Useless Work for Bluestein Kernels
    \medskip
  \item Fuse Kernels for Multi-dimensional Transforms
  \end{itemize}
...
\begin{frame}
  ...
  Suppose we can fit $M\sqrt{N}P$ values into cache.

  Then, we could
  \begin{enumerate}
  \item Read from global
  \item Perform $MN$ length-$P$ transforms
  \item Perform the $MP$ initial radix-$\sqrt{N}$ sub-transforms
  \item Write to global
  \item Read from global
  \item Perform the $MP$ final radix-$\sqrt{N}$ sub-transforms
  \item Perform $NP$ length-$M$ transforms
  \item Write to global
  \end{enumerate}
