...
pen donepen = green;
pen implementedpen = paleblue;
pen notdonepen = lightgrey;

// from left to right and top to bottom
block spectralapi = roundrectangle(Label("Spectral Ops API"), (xop,0), donepen);
block multiapi    = roundrectangle(Label("MultiGPU API"),     (0,0), donepen);
block sparseapi   = roundrectangle(Label("Sparsity API"),     (xsparse,0), donepen);

block api         = roundrectangle(Label("API Proposal"),     (0,-dy), donepen);

block oneway = roundrectangle(Label("One-way ops"), (xop,-2dy), lightgrey);
block onewayfields = roundrectangle(Label("Multi-valued one-way"), (xop,-3dy), lightgrey);
block twoway = roundrectangle(Label("Round-trip unpadded ops"), (xop,-4dy), lightgrey);
block twowaypad = roundrectangle(Label("Convolution"), (xop,-5dy), lightgrey);

block fields = roundrectangle(Label("Fields"), (0,-2dy), donepen);
block brickdata = roundrectangle(Label("Brick decomposition"), (0,-3dy), donepen);
...
block singlenode = roundrectangle(Label("Single-node multi-gpu"), (-0.8dx,-4dy),
                                  implementedpen);
block multinode = roundrectangle(Label("Multi-node MPI"), (0,-5dy), implementedpen);
block multinodeschmem = roundrectangle(Label("Multi-node SHMEM"), (dx,-6dy), lightgrey);

block sparse = roundrectangle(Label("Sparse data"), (xsparse,-4dy), lightgrey);
